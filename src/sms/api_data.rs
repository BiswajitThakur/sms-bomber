...
impl ApiData {
    pub fn filter(self, mobile: u64) -> Self {
        ...
        let mut query: HashMap<String, String> = HashMap::with_capacity(self.query.capacity());
        for (k, v) in &self.query {
            query.insert(k.to_string(), re.replace_all(v, &mobile).to_string());
        }
        let body = re.replace_all(&self.body, &mobile).to_string();
